...
import {BasketParamsList} from "@screens/GuestNavigator/Screens/basket/BasketNavigator";
...
export const BasketSingle: React.FC = () => {
    ...
    useEffect(() => {
        navigation.setOptions({
            ...
            headerLeft: () => <ModalCloseIcon onPress={() => navigation.navigate<any>(BasketScreenName.BASKET)} />,
        })
    }, [])
    ...
    const onPress = () => {
        ...
        navigation.navigate<any>(OnboardingScreenName.ONBOARDING, {screen: OnboardingScreenName.ENTER_MOBILE_PHONE})
    }
    ...
    return (
       <View style={tailwind('flex-1 bg-white relative px-4')}>
           ...
           <GenericButton style={tailwind('absolute bottom-0 w-full self-center mb-10')} onPress={onPress} label="Create an account to checkout" labelColor={tailwind('font-medium text-white')} backgroundColor={tailwind('bg-black')} testId="" />
       </View>
    )
}
