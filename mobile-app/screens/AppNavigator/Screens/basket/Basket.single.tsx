...
export const BasketSingle: React.FC = () => {
    ...
    return (
       <View style={tailwind('flex-1 bg-white relative px-4')}>
           <ScrollView style={tailwind('flex-1 flex-col ')}>
               <View>
                   <View style={tailwind('flex flex-col my-3')}>
                       {cart !== undefined && cart.cart?.map((_cart, index) => (
           ...
           <GenericButton style={tailwind('absolute bottom-0 w-full self-center mb-2')} onPress={() => navigation.navigate(BasketScreenName.CHECKOUT)} label="Go to checkout" labelColor={tailwind('font-medium text-white')} backgroundColor={tailwind('bg-black')} testId="" />
       </View>
    )
}
