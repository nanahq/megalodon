    ...
    return (
        <View style={tailwind('flex flex-row items-center justify-between w-full mb-2', {'mt-2 border-t-0.5 border-slate-200 pt-2': props.subTotal})}>
           <View style={tailwind('flex flex-row items-center')}>
               <Text style={tailwind('text-slate-900 font-normal text-sm capitalize', {'font-bold': props.subTotal})}>
                   {props.label}
               </Text>
            ...
            <NumberFormat
                ...
                displayType="text"
                renderText={(value) => (
                    <Text style={tailwind("text-sm text-slate-900 font-normal", {'font-semibold': props.subTotal})}>{value}</Text>
                )}
            />
        </View>
    )
