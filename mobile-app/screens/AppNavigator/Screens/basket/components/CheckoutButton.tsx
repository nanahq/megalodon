       ...
       <View style={tailwind('mt-4 flex flex-col')}>
           ...
           <View style={tailwind('bg-gray-100 p-2 rounded-lg')}>
               <View style={tailwind('flex flex-row items-center justify-between w-full')}>
                   <Pressable  disabled={vendorType === 'PRE_ORDER'} onPress={() => onButtonClick("ON_DEMAND")} style={tailwind('w-1/2 rounded-lg py-2 px-12', {'bg-primary-500 text-white': view === 'ON_DEMAND'})}>
                       <Text style={tailwind('text-base', {'text-white': view === 'ON_DEMAND'})}>Deliver Now</Text>
                   </Pressable>
                   <Pressable disabled={vendorType === 'ON_DEMAND'} onPress={() => onButtonClick("PRE_ORDER")} style={tailwind('w-1/2 text-lg rounded-lg py-2 px-12', {'bg-primary-500 text-white': view === 'PRE_ORDER'})}>
