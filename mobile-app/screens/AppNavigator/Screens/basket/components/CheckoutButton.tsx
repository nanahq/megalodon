       ...
       <View style={tailwind('mt-4 flex flex-col')}>
           ...
           <View style={tailwind('bg-brand-ash p-2 rounded-40')}>
               <View style={tailwind('flex flex-row items-center justify-between w-full')}>
                   <Pressable  disabled={vendorType === 'PRE_ORDER'} onPress={() => onButtonClick("ON_DEMAND")} style={tailwind('rounded-40 py-2 px-12', {'bg-white ': view === 'ON_DEMAND'})}>
                       <Text>Deliver Now</Text>
                   </Pressable>
                   <Pressable disabled={vendorType === 'ON_DEMAND'} onPress={() => onButtonClick("PRE_ORDER")} style={tailwind('rounded-40 py-2 px-12', {'bg-white ': view === 'PRE_ORDER'})}>
                       <Text>Pre Order</Text>
                   </Pressable>
               </View>
           </View>
